...
app.get('/ebay/search', async (req, res) => {
    ...
    try {
        ...
        const items = parsedData.findItemsAdvancedResponse[0].searchResult[0].item;
        
        
        const extractedData = items.map(item => {
            let shippingCost = item.shippingInfo[0].shippingServiceCost[0]['__value__'];
            if (shippingCost === "0.0" || shippingCost === "0") {  // 检查为字符串或数字0的情况
            ...
            return {
                imageURL: item.galleryURL?.[0] || "not found",
                title: item.title?.[0] || "not found",
                price: item.sellingStatus?.[0].currentPrice?.[0]['__value__'] || "not found",
                postalCode: item.postalCode?.[0] || "not found",
                conditionDisplayName: item.condition?.[0].conditionDisplayName?.[0] || "not found",
                shippingCost: shippingCost|| "not found",
                shippingLocations: item.shippingInfo?.[0].shipToLocations?.[0] || "not found",
                handlingTime: item.shippingInfo?.[0].handlingTime?.[0] || "not found",
                expeditedShipping: item.shippingInfo?.[0].expeditedShipping?.[0] || "not found",
                oneDayShipping: item.shippingInfo?.[0].oneDayShippingAvailable?.[0] || "not found",
                returnsAccepted: item.returnsAccepted?.[0] || "not found",
                link: item.viewItemURL?.[0] || "not found",
                itemID: item.itemId?.[0] || "not found",
                categoryId: item.primaryCategory?.[0].categoryId?.[0] || "not found",
                // categoryName: item.primaryCategory[0].categoryName[0],  // 如果需要，可以取消注释此行以提取categoryName
            };
        ...
        const refinedData = extractedData.map(item => {
            return {
                ...
                shippingLocations: item.shippingLocations,  // 获取Shipping Locations
                handlingTime: item.handlingTime,  // 获取Handling Time
                expeditedShipping: item.expeditedShipping,  // 获取Expedited Shipping
                oneDayShipping: item.oneDayShipping,  // 获取One day Shipping
                returnsAccepted: item.returnsAccepted  // 获取Return Accepted
            };
        });
...
app.get('/getSingleItem', async (req, res) => {
    ...
    try {
        ...
        let extractedData = {
            ...
            returnPolicy: (itemData.Item.ReturnPolicy && itemData.Item.ReturnPolicy.ReturnsAccepted && itemData.Item.ReturnPolicy.ReturnsWithin) ? itemData.Item.ReturnPolicy.ReturnsAccepted + " for " + itemData.Item.ReturnPolicy.ReturnsWithin : "not found",
...
async function findSimilarItems(item_id) {
    const app_id = EBAY_APP_KEY;
    // 假设有一个名为FindSimilarItems的eBay API
...
app.post('/saveItem', async (req, res) => {
 ...
 app.get('/getWishList', async (req, res) => {
    ...
    try {
      ...
      // 明确地设置 Content-Type 和使用 res.json() 发送数据
